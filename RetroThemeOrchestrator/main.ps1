...
function Write-Log {
    ...
    $Form.Refresh()
    # In a future version, this could also write to a log file.
}
...
function Install-Tools {
    ...
    if (-not (Test-Path $secureUxThemePath)) {
        $secureInstaller = Join-Path $ToolsDirectory "SecureUxTheme.msi"
        if (-not (Test-Path $secureInstaller)) {
            if (-not (Download-Tool -Url "https://github.com/namazso/SecureUxTheme/releases/download/v4.0.0/SecureUxTheme.msi" -OutFile $secureInstaller -ToolName "SecureUxTheme")) { return $false }
        }
    ...
    if (-not (Test-Path $retroBarPath)) {
        $retroInstaller = Join-Path $ToolsDirectory "RetroBar.Installer.exe"
        if (-not (Test-Path $retroInstaller)) {
            if (-not (Download-Tool -Url "https://github.com/dremin/RetroBar/releases/download/v1.20.37/RetroBar.Installer.exe" -OutFile $retroInstaller -ToolName "RetroBar")) { return $false }
        }
    ...
    if (-not (Test-Path $openShellPath)) {
        $openInstaller = Join-Path $ToolsDirectory "OpenShellSetup.exe"
        if (-not (Test-Path $openInstaller)) {
            if (-not (Download-Tool -Url "https://github.com/Open-Shell/Open-Shell-Menu/releases/download/v4.4.196/OpenShellSetup_4_4_196.exe" -OutFile $openInstaller -ToolName "Open-Shell")) { return $false }
        }
